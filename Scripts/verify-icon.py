...
def validate_compiled_layout(items):
    ...
    for item in items:
        ...
        if item.get("AssetType") == "IconGroup":
            assert item["Name"] == "AppIcon/Circle", "Split foreground material groups returned"
            assert item["LayerCount"] == 1 and len(item["Layers"]) == 1, "Circle must be one artwork layer"
            assert item["Layers"][0]["AssetType"] == "Vector", "Circle vector missing"
    for stack in stacks:
        ...
        assert foreground and all(layer["Name"] == "AppIcon/Circle" for layer in foreground), "Split foreground material groups returned"
...
def main():
    ...
    assert len(groups) == 1 and groups[0]["name"] == "Circle", "Expected one shared foreground material group"
    group = groups[0]
    assert group["layers"] == [{"image-name": "Circle.svg", "name": "Circle"}], "Circle must be one artwork layer"
    assert group["translucency"]["enabled"]
    assert group["specular"] is True, "Shared foreground glass is disabled"
    svg = ET.parse(source / "Assets/Circle.svg").getroot()
    assert svg.attrib["viewBox"] == "0 0 1024 1024"
    assert svg.attrib["width"] == svg.attrib["height"] == "1024"
    assert len(svg) == 1 and svg[0].tag.endswith("}circle"), "Expected one complete circular ring"
    assert svg[0].attrib == {"cx": "512", "cy": "512", "r": "266", "fill": "none", "stroke": "#c3cbd6", "stroke-width": "70"}, "Circle geometry or silver color changed"
    assert document["fill"] == {"solid": "extended-srgb:0.10980,0.10980,0.11765,1.00000"}, "Dark-only backplate changed"
